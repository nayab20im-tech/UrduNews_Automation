        ...
        for entry in parsed.entries:
            ...
            for mc in media_content:
                if mc.get("medium") == "image" or mc.get("type", "").startswith("image/"):
                    image_url = image_url or mc.get("url", "")
                elif mc.get("medium") == "video" or mc.get("type", "").startswith("video/"):
                    video_url = video_url or mc.get("url", "")

            # 2. Enclosures
            for enc in getattr(entry, "enclosures", []):
                if enc.get("type", "").startswith("image/"):
                    image_url = image_url or enc.get("href", "")
                elif enc.get("type", "").startswith("video/"):
                    video_url = video_url or enc.get("href", "")

            # 3. HTML extraction (from summary/content)
            if not image_url or not video_url:
                import re
                html_body = getattr(entry, "summary", "") + str(entry.get("content", [{"value": ""}])[0].get("value", ""))
                if not image_url:
                    img_match = re.search(r'<img[^>]+src=["\']([^"\']+)["\']', html_body, re.IGNORECASE)
                    if img_match:
                        image_url = img_match.group(1)
                if not video_url:
                    vid_match = re.search(r'<video[^>]+src=["\']([^"\']+)["\']', html_body, re.IGNORECASE)
                    if vid_match:
            ...
            articles.append(
                self._build_article(
                    ...
                    extra={"feed_name": feed_name, "raw_categories": categories, "image_url": image_url, "video_url": video_url},
                )
            )
        return articles
